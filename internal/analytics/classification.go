...
cov := 0.0
if mu > 0 { cov = stdDev / mu }

xyz := "X"
if cov > 1.0 { xyz = "Z" } else if cov > 0.5 { xyz = "Y" }
...
if totalValue > 0 { cumulative += item.AnnualValue / totalValue }
abc := "C"
if cumulative <= 0.80 { abc = "A" } else if cumulative <= 0.95 { abc = "B" }
